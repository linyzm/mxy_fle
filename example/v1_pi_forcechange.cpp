...
// t 从进入 Stage 2 开始计时（秒）。未指定的前 0.5 s 沿用 10 N。
// 0 <= t <= 2: 10 N; t > 2: 10 + 5*sin(t-2) N，角频率为 1 rad/s。
double DesiredForceZ(double force_control_time)
{
    if (force_control_time <= 2.0) {
        return kDesiredForceZ;
    }
    return kDesiredForceZ + 5.0 * std::sin(force_control_time - 2.0);
}
    ...
    bool enable_collision)
{
    ...
    static double stage2_start_time = 0.0;
    ...
    try {
        ...
        const double desired_force_z = stage == ControlStage::ForceControl
            ? DesiredForceZ(time - stage2_start_time) : kDesiredForceZ;
        const double force_error = measured_force_z - desired_force_z;

        //导出力
        if (log_file.is_open() && loop_counter % kLogInterval == 0) {
            log_file << time << "," << measured_force_z << "," << desired_force_z << "\n";
        }
        ...
        case ControlStage::ForceControl:
        {
            ...
            const double offset_z = admittance_z.Update(desired_force_z, measured_force_z);
            // 默认认为：
            ...
            if (loop_counter % kLoopFreq == 0) {
                spdlog::info(
                    "Stage 2 | Fz: {:.3f} N, Fd: {:.3f} N, Ferr: {:.3f} N, "
...
int main(int argc, char* argv[])
{
    ...
    spdlog::info(
        ...
        "Stage 2: track a time-varying force with Z-direction PI admittance control.\n");
